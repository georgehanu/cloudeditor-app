	...
	function FocusManager(editorManager) {
		...
		function createBookmark(dom, rng) {
			if (rng && rng.startContainer) {
				...
				if (
					!dom.isChildOf(rng.startContainer, dom.getRoot()) ||
					!dom.isChildOf(rng.endContainer, dom.getRoot())
				) {
					return;
				}
		...
		function registerEvents(e) {
			...
			editor.on("focusin", function() {
				...
				if (editor.selection.lastFocusBookmark) {
					editor.selection.setRng(
						bookmarkToRng(editor, editor.selection.lastFocusBookmark)
					);
					editor.selection.lastFocusBookmark = null;
				}
				...
				documentFocusInHandler = function(e) {
					...
					if (activeEditor && e.target.ownerDocument == document) {
						...
						if (activeEditor.selection && e.target != activeEditor.getBody()) {
							activeEditor.selection.lastFocusBookmark = createBookmark(
								activeEditor.dom,
								activeEditor.lastRng
							);
						}
			...
			if (editor.inline && !documentMouseUpHandler) {
				documentMouseUpHandler = function(e) {
					...
					if (
						activeEditor.inline &&
						!activeEditor.dom.isChildOf(e.target, activeEditor.getBody())
					) {
						var rng = activeEditor.selection.getRng();
