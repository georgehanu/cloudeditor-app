...
define("tinymce/ui/Window", [
	...
	function toggleBodyFullScreenClasses(classPrefix) {
		...
		$([document.documentElement, document.body]).removeClass(
	...
	var Window = FloatPanel.extend({
		...
		initLayoutRect: function() {
			...
			layoutRect.x =
				self.settings.x || Math.max(0, rect.w / 2 - layoutRect.w / 2);
			layoutRect.y =
				self.settings.y || Math.max(0, rect.h / 2 - layoutRect.h / 2);
		...
		fullscreen: function(state) {
			...
			if (state != self._fullscreen) {
				...
				if (!state) {
					...
					$([documentElement, document.body]).removeClass(
						prefix + "fullscreen"
					);
					self.classes.remove("fullscreen");
					self
						.moveTo(self._initial.x, self._initial.y)
						.resizeTo(self._initial.w, self._initial.h);
				} else {
					self._initial = {
						x: layoutRect.x,
						y: layoutRect.y,
