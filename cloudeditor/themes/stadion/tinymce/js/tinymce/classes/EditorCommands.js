...
define("tinymce/EditorCommands", [
	...
	return function(editor) {
		...
		function execCommand(command, ui, value, args) {
			...
			if (
				editor.theme &&
				editor.theme.execCommand &&
				editor.theme.execCommand(command, ui, value)
			) {
				editor.fire("ExecCommand", { command: command, ui: ui, value: value });
				return true;
			}
		...
		addCommands({
			...
			mceInsertContent: function(command, ui, value) {
				...
				function trimOrPaddLeftRight(html) {
					...
					function hasSiblingText(siblingName) {
						return (
							container[siblingName] && container[siblingName].nodeType == 3
						);
					}
				...
				var caretElement =
					rng.startContainer ||
					(rng.parentElement ? rng.parentElement() : null);
				...
				if (
					...
					each(selection.getSelectedBlocks(), function(element) {
						if (element.nodeName != "LI") {
							var indentStyleName = editor.getParam("indent_use_margin", false)
								? "margin"
								: "padding";

							indentStyleName +=
								dom.getStyle(element, "direction", true) == "rtl"
									? "Right"
									: "Left";
							...
							if (command == "outdent") {
								value = Math.max(
									0,
									parseInt(element.style[indentStyleName] || 0, 10) -
										intentValue
								);
								dom.setStyle(
									element,
			...
			InsertLineBreak: function(command, ui, value) {
				...
				if (container.nodeType == 1 && container.hasChildNodes()) {
					var isAfterLastNodeInContainer =
				...
				var parentBlockName = parentBlock
				...
				Outdent: function() {
					...
					if (settings.inline_styles) {
						if (
							...
							parseInt(node.style.paddingLeft, 10) > 0
						) {
							return TRUE;
						}

						if (
							(node = dom.getParent(selection.getEnd(), dom.isBlock)) &&
							parseInt(node.style.paddingLeft, 10) > 0
						) {
							return TRUE;
						}
