	...
	Selection.prototype = {
		...
		getContent: function(args) {
			...
			if (args.format == "text") {
				return self.isCollapsed()
					? ""
					: rng.text || (se.toString ? se.toString() : "");
			}
			...
			if (rng.cloneContents) {
				...
				tmpElm.innerHTML =
					"<br>" + (rng.item ? rng.item(0).outerHTML : rng.htmlText);
			...
			args.content = self.isCollapsed()
				? ""
				: whiteSpaceBefore +
				  self.serializer.serialize(tmpElm, args) +
				  whiteSpaceAfter;
		...
		getStart: function(real) {
			...
			if (startElement.nodeType == 1 && startElement.hasChildNodes()) {
				if (!real || !rng.collapsed) {
					startElement =
						startElement.childNodes[
							Math.min(startElement.childNodes.length - 1, rng.startOffset)
						];
				}
			}
		...
		getEnd: function(real) {
			...
			if (endElement.nodeType == 1 && endElement.hasChildNodes()) {
				if (!real || !rng.collapsed) {
					endElement =
						endElement.childNodes[endOffset > 0 ? endOffset - 1 : endOffset];
				}
			}
		...
		getRng: function(w3c) {
			...
			if (self.selectedRange && self.explicitRange) {
				if (
					tryCompareBoundaryPoints(
						rng.START_TO_START,
						rng,
						self.selectedRange
					) === 0 &&
					tryCompareBoundaryPoints(rng.END_TO_END, rng, self.selectedRange) ===
						0
				) {
				...
				if (!rng.collapsed) {
					...
					if (startContainer.nodeType === 3 && endContainer.nodeType === 3) {
						if (startContainer.length === startOffset) {
							startContainer = skipEmptyTextNodes(
		...
		scrollIntoView: function(elm) {
			...
			if (y < viewPort.y || y + 25 > viewPortY + viewPortH) {
				self.editor
					.getWin()
					.scrollTo(0, y < viewPortY ? y : y - viewPortH + 25);
			}
		},
		...
		_moveEndPoint: function(rng, node, start) {
			...
			do {
				...
				if (
					Env.ie &&
					Env.ie < 11 &&
					this.dom.isBlock(node) &&
					this.dom.isEmpty(node)
				) {
					if (start) {
						rng.setStart(node, 0);
					} else {
