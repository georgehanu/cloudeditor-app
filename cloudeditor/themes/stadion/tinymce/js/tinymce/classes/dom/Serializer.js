	...
	return function(settings, editor) {
		...
		htmlParser.addAttributeFilter("data-mce-type", function(nodes, name, args) {
			...
			while (i--) {
				...
				if (
					node.attributes.map["data-mce-type"] === "bookmark" &&
					!args.cleanup
				) {
					node.remove();
				}
		...
		return {
			...
			serialize: function(node, args) {
				...
				if (impl.createHTMLDocument) {
					...
					each(node.nodeName == "BODY" ? node.childNodes : [node], function(
						node
					) {
				...
				rootNode = htmlParser.parse(
					trim(args.getInner ? node.innerHTML : dom.getOuterHTML(node)),
					args
				);
