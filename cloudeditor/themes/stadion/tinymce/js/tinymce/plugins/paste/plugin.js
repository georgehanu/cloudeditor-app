...
define("tinymce/pasteplugin/Clipboard", [
	...
	return function(editor) {
		...
		function pasteText(text) {
			...
			var startBlock = editor.dom.getParent(
			...
			if (forcedRootBlockName) {
				forcedRootBlockStartHtml = editor.dom.createHTML(
					forcedRootBlockName,
					editor.settings.forced_root_block_attrs
				);
				forcedRootBlockStartHtml =
					forcedRootBlockStartHtml.substr(
						0,
						forcedRootBlockStartHtml.length - 3
					) + ">";
			}

			if (
				(startBlock && /^(PRE|DIV)$/.test(startBlock.nodeName)) ||
				!forcedRootBlockName
			) {
		...
		function registerEventHandlers() {
			...
			editor.on("paste", function(e) {
				...
				var isKeyBoardPaste =
					new Date().getTime() - keyboardPasteTimeStamp - clipboardDelay < 1000;
				var plainTextMode =
