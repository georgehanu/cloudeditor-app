...
define("tinymce/html/Schema", ["tinymce/util/Tools"], function(Tools) {
	...
	function compileSchema(type) {
		...
		globalAttributes = split(
			"id accesskey class dir lang style tabindex title"
		);
	...
	return function(settings) {
		...
		each(
			(settings.special || "script noscript style textarea").split(" "),
			function(name) {
		...
		if (!settings.valid_elements) {
			...
			each(split("p h1 h2 h3 h4 h5 h6 th td pre div address caption"), function(
				name
			) {
