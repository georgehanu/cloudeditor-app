...
define("tinymce/dom/DOMUtils", [
	...
	function setupAttrHooks(domUtils, settings) {
		var attrHooks = {},
			keepValues = settings.keep_values,
			keepUrlHook;
			...
			set: function($elm, value, name) {
				if (settings.url_converter) {
					value = settings.url_converter.call(
						settings.url_converter_scope || domUtils,
						value,
						name,
						$elm[0]
					);
		...
		attrHooks = {
			style: {
				...
				get: function($elm) {
					...
					value = domUtils.serializeStyle(
						domUtils.parseStyle(value),
						$elm[0].nodeName
					);
	...
	function DOMUtils(doc, settings) {
		...
		self.fixDoc(doc);
		self.events = settings.ownEvents
			? new EventUtils(settings.proxy)
			: EventUtils.Event;
		self.attrHooks = setupAttrHooks(self, settings);
		blockElementsMap = settings.schema
			? settings.schema.getBlockElements()
			: {};
	...
	DOMUtils.prototype = {
		...
		getParents: function(node, selector, root, collect) {
			...
			root =
				root ||
				(self.getRoot().nodeName != "BODY" ? self.getRoot().parentNode : null);
		...
		select: function(selector, scope) {
			...
			return Sizzle(
				selector,
				self.get(scope) || self.settings.root_element || self.doc,
				[]
			);
		},
		...
		getOuterHTML: function(elm) {
			...
			return elm.nodeType == 1 && "outerHTML" in elm
				? elm.outerHTML
				: $("<div>")
						.append($(elm).clone())
						.html();
			...
			if (elm.nodeName != name.toUpperCase()) {
				...
				each(self.getAttribs(elm), function(attrNode) {
					self.setAttrib(
						newElm,
						attrNode.nodeName,
						self.getAttrib(elm, attrNode.nodeName)
					);
				});
		...
		isEmpty: function(node, elements) {
			...
			if (node) {
				...
				elements =
					elements || (self.schema ? self.schema.getNonEmptyElements() : null);
		...
		split: function(parentElm, splitElm, replacementElm) {
			...
			function trimNode(node) {
				...
				function surroundedBySpans(node) {
					...
					var nextIsSpan =
						node.nextSibling && node.nextSibling.nodeName == "SPAN";
				...
				if (type != 9) {
					...
					if (type == 3 && node.nodeValue.length > 0) {
						...
						if (
							!self.isBlock(node.parentNode) ||
							trimmedLength > 0 ||
							(trimmedLength === 0 && surroundedBySpans(node))
						) {
							return;
						}
					} else if (type == 1) {
		...
		bind: function(target, name, func, scope) {
			...
			if (
				...
				(target === self.doc || target === self.win)
			) {
