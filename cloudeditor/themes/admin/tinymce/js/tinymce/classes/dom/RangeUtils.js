	...
	function RangeUtils(dom) {
		...
		this.walk = function(rng, callback) {
			...
			function walkBoundary(start_node, end_node, next) {
				...
				for (
					node = start_node, parent = node.parentNode;
					node && node != end_node;
					node = parent
				) {
					parent = node.parentNode;
					siblings = collectSiblings(
						node == start_node ? node : node[siblingName],
						siblingName
					);
			...
			if (startContainer == endContainer && startContainer.nodeType == 3) {
				if (startOffset > 0 && startOffset < startContainer.nodeValue.length) {
					...
					if (endOffset > startOffset) {
						...
						startContainer = endContainer = splitText(endContainer, endOffset)
							.previousSibling;
		...
		this.normalize = function(rng) {
			...
			function normalizeEndPoint(start) {
				...
				isAfterNode =
					container.nodeType == 1 && offset === container.childNodes.length;
				...
				if (collapsed) {
					...
					if (container.nodeType === 1) {
						...
						if (
							...
							node.nodeName === "BR" &&
							!isPrevNode(node, "A") &&
							!hasBrBeforeAfter(node) &&
							!hasBrBeforeAfter(node, true)
						) {
