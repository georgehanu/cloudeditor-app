	...
	return function(ed) {
		...
		function processUnderlineAndColor(node) {
			...
			if (
				...
				if (ed.dom.getStyle(node, "color") && textDecoration) {
					...
					ed.dom.getStyle(node, "text-decoration") === textDecoration
				) {
		...
		function apply(name, vars, node) {
			var formatList = get(name),
				...
				bookmark,
				rng,
				isCollapsed = !node && selection.isCollapsed();
			...
			function adjustSelectionToVisibleSelection() {
				...
				if (start != end && rng.endOffset === 0) {
					...
					var endOffset =
						newEnd.nodeType == 3 ? newEnd.length : newEnd.childNodes.length;
			...
			if (format) {
				if (node) {
					...
					if (
						!isCollapsed ||
						!format.inline ||
						dom.select("td.mce-item-selected,th.mce-item-selected").length
					) {
						...
						applyRngStyle(
							expandRng(selection.getRng(TRUE), formatList),
							bookmark
						);
			...
			function removeRngStyle(rng) {
				...
				if (format.split) {
					...
					if (startContainer != endContainer) {
						...
						if (
							dom.isChildOf(startContainer, endContainer) &&
							!isBlock(endContainer) &&
							!isTableCell(startContainer) &&
							!isTableCell(endContainer)
						) {
		...
		function matchNode(node, name, vars, similar) {
						...
						for (key in items) {
							if (items.hasOwnProperty(key)) {
								...
								if (
									...
									!isEq(
										value,
										normalizeStyleValue(replaceVars(items[key], vars), key)
									)
								) {
									return;
								}
			...
			if (formatList && node) {
				...
				for (i = 0; i < formatList.length; i++) {
					...
					if (
						matchName(node, format) &&
						matchItems(node, format, "attributes") &&
		...
		function match(name, vars, node) {
			...
			function matchParents(node) {
				...
				node = dom.getParent(node, function(node) {
					return (
						node.parentNode === root || !!matchNode(node, name, vars, true)
					);
				});
		...
		function formatChanged(formats, callback, similar) {
			...
			if (!formatChangeData) {
				...
				ed.on("NodeChange", function(e) {
					var parents = getParents(e.element),
						matchedFormats = {};
		...
		function expandRng(rng, format, remove) {
			...
			function findParentContainer(start) {
				...
				if (container.nodeType == 3 && !isWhiteSpaceNode(container)) {
					if (
						start ? startOffset > 0 : endOffset < container.nodeValue.length
					) {
						return container;
					}
				}
			...
			if (startContainer.nodeType == 1 && startContainer.hasChildNodes()) {
				...
				startContainer =
					startContainer.childNodes[
						startOffset > lastIdx ? lastIdx : startOffset
					];
			...
			if (endContainer.nodeType == 1 && endContainer.hasChildNodes()) {
				...
				endContainer =
					endContainer.childNodes[
						endOffset > lastIdx ? lastIdx : endOffset - 1
					];
			...
			function findWordEndPoint(container, offset, start) {
				...
				walker = new TreeWalker(
					container,
					dom.getParent(container, isBlock) || ed.getBody()
				);
			...
			if (
				isBookmarkNode(startContainer.parentNode) ||
			...
			if (
				isBookmarkNode(endContainer.parentNode) ||
			...
			if (format[0].inline || format[0].block_expand) {
				if (
					!format[0].inline ||
					(startContainer.nodeType != 3 || startOffset === 0)
				) {
			...
			if (format.remove != "all") {
				...
				each(format.styles, function(value, name) {
					...
					if (
						...
						(!compare_node || isEq(getStyle(compare_node, name), value))
					) {
		...
		function performCaretAction(type, name, vars, similar) {
			...
			function createCaretContainer(fill) {
				...
				if (fill) {
					caretContainer.appendChild(
						ed.getDoc().createTextNode(INVISIBLE_CHAR)
					);
				}
			...
			function isCaretContainerEmpty(node, nodes) {
				while (node) {
					if (
						(node.nodeType === 3 && node.nodeValue !== INVISIBLE_CHAR) ||
						node.childNodes.length > 1
					) {
						return false;
					}
			...
			function applyCaretFormat() {
				...
				if (
					...
					offset > 0 &&
					offset < text.length &&
					/\w/.test(text.charAt(offset)) &&
					/\w/.test(text.charAt(offset - 1))
				) {
			...
			if (!ed._hasCaretEvents) {
				...
				disableCaretContainer = function(e) {
					...
					if (
						keyCode == 8 &&
						selection.isCollapsed() &&
						selection.getStart().innerHTML == INVISIBLE_CHAR
					) {
