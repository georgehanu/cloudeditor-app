						...
						if (++initCount == editors.length) {
							if (typeof func === "string") {
								scope =
									func.indexOf(".") === -1
										? null
										: tinymce.resolve(func.replace(/\.\w+$/, ""));
		...
		if (!win.tinymce && !lazyLoading && (url = settings.script_url)) {
			...
			if (url.indexOf("gzip") != -1) {
				...
				url =
					...
					escape(suffix) +
					"&themes=" +
					escape(settings.theme || "modern") +
					"&plugins=" +
					escape(settings.plugins || "") +
					"&languages=" +
					(lang || "");
			...
			script.onload = script.onreadystatechange = function(e) {
				...
				if (
					lazyLoading !== 2 &&
					(e.type == "load" || /complete|loaded/.test(script.readyState))
				) {
		...
		$.each(["text", "html", "val"], function(i, name) {
			...
			$.fn[name] = function(value) {
				...
				(textProc ? self : self.eq(0)).each(function(i, node) {
					...
					if (ed) {
						ret += textProc
							? ed.getContent().replace(/<(?:"[^"]*"|'[^']*'|[^'">])*>/g, "")
							: ed.getContent({ save: true });
					} else {
				...
				if (value !== undef) {
					if (typeof value === "string") {
						self.filter(":tinymce").each(function(i, node) {
							...
							if (ed) {
								ed.setContent(
									prepend ? value + ed.getContent() : ed.getContent() + value
								);
							}
						});
					}
