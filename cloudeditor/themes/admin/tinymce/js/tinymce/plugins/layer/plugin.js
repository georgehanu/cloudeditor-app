...
tinymce.PluginManager.add("layer", function(editor) {
	...
	function move(d) {
		var i,
			z = [],
			le = getParentLayer(editor.selection.getNode()),
			ci = -1,
			fi = -1,
			nl;
		...
		tinymce.walk(
			...
			function(n) {
				if (
					n.nodeType == 1 &&
					/^(absolute|relative|static)$/i.test(n.style.position)
				) {
					nl.push(n);
				}
			},
	...
	function insertLayer() {
		var dom = editor.dom,
			p = dom.getPos(dom.getParent(editor.selection.getNode(), "*"));
	...
	// Register buttons
	editor.addButton("moveforward", {
		title: "layer.forward_desc",
		cmd: "mceMoveForward"
	});
	editor.addButton("movebackward", {
		title: "layer.backward_desc",
		cmd: "mceMoveBackward"
	});
	editor.addButton("absolute", {
		title: "layer.absolute_desc",
		cmd: "mceMakeAbsolute"
	});
	editor.addButton("insertlayer", {
		title: "layer.insertlayer_desc",
		cmd: "mceInsertLayer"
	});
