...
tinymce.PluginManager.add("noneditable", function(editor) {
	...
	var externalName = "contenteditable",
		internalName = "data-mce-" + externalName;
	...
	function handleContentEditableSelection() {
		var dom = editor.dom,
			...
			caretContainerId = "mce_noneditablecaret",
			invisibleChar = "\uFEFF";
		...
		function insertCaretContainerOrExpandToBlock(target, before) {
			...
			*/
			...
			caretContainer = dom.create(
				"span",
				{ id: caretContainerId, "data-mce-bogus": true },
				invisibleChar
			);
		...
		function removeCaretContainer(caretContainer) {
			...
			if (caretContainer) {
				...
				while (
					(caretContainer = dom.get(caretContainerId)) &&
					caretContainer !== lastContainer
				) {
					child = findFirstTextNode(caretContainer);
					if (child && child.nodeValue.charAt(0) == invisibleChar) {
						child = child.deleteData(0, 1);
		...
		function handleKey(e) {
			...
			function canDelete(backspace) {
				...
				if (selection.isCollapsed()) {
					...
					if (
						...
						(backspace ? offset > 0 : offset < container.nodeValue.length)
					) {
						return true;
					}
			...
			nonEditableParent =
				getNonEditableParent(startElement) || getNonEditableParent(endElement);
			...
			if (
				...
				if (
					...
					(keyCode == 67 || keyCode == 88 || keyCode == 86)
				) {
					return;
				}
				...
				if (isDirectionKey) {
					...
					if (editor.dom.isBlock(nonEditableParent)) {
						var targetElement = left
							? nonEditableParent.previousSibling
							: nonEditableParent.nextSibling;
						...
						if (
							!targetElement ||
							(targetElement && getContentEditable(targetElement) === "false")
						) {
				...
				if (isDirectionKey || keyCode == VK.BACKSPACE || keyCode == VK.DELETE) {
					...
					if (caretContainer) {
						...
						if (keyCode == VK.LEFT || keyCode == VK.BACKSPACE) {
							nonEditableParent = getNonEmptyTextNodeSibling(
								caretContainer,
								true
							);
							...
							if (
								nonEditableParent &&
								getContentEditable(nonEditableParent) === "false"
							) {
								e.preventDefault();
						...
						if (isDirectionKey) {
							...
							if (
								currentNode &&
								currentNode.className.indexOf("mceTmpParagraph") !== -1 &&
								...
								if (
									...
									currentNode.className = currentNode.className.replace(
										"mceTmpParagraph",
										""
									);
								}
						...
						if (
							dom.isBlock(container) &&
							dom.isBlock(container.nextSibling) &&
							rng.endOffset == 1 &&
							keyCode == VK.DELETE
						) {
						...
						if (
							nonEditableParent &&
							(keyCode == VK.DELETE || keyCode == VK.BACKSPACE) &&
							dom.isBlock(nonEditableParent)
						) {
							e.preventDefault();
							dom.remove(nonEditableParent);
							return;
					...
					if (
						(keyCode == VK.BACKSPACE || keyCode == VK.DELETE) &&
						!canDelete(keyCode == VK.BACKSPACE)
					) {
						e.preventDefault();
						return false;
					}
		...
		editor.on("mousedown", function(e) {
			...
			if (
				node &&
				node.className.indexOf("mceTmpParagraph") !== -1 &&
				node !== e.target
			) {
				// current node is still empty and a separator -> remove it
				// else: remove the separator class, as it now includes content
	...
	editClass =
		" " +
		tinymce.trim(editor.getParam("noneditable_editable_class", "mceEditable")) +
		" ";
	nonEditClass =
		" " +
		tinymce.trim(
