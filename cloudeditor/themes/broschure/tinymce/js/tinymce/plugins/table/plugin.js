 ...
 * @private
 */
define("tinymce/tableplugin/Utils", ["tinymce/Env"], function(Env) {
...
define("tinymce/tableplugin/TableGrid", [
	...
	return function(editor, table) {
		...
		function isCellSelected(cell) {
			return (
				cell &&
				(dom.hasClass(cell.elm, "mce-item-selected") || cell == selectedCell)
			);
		}
		...
		function getSelectedRows() {
			...
			each(table.rows, function(row) {
				each(row.cells, function(cell) {
					if (
						dom.hasClass(cell, "mce-item-selected") ||
						...
						if (cell != startCell) {
							...
							if (children.length) {
								...
								each(children, function(node) {
									if (
										node.nodeName == "BR" &&
										dom.getAttrib(node, "data-mce-bogus") &&
		...
		function pasteRows(rows, before) {
			...
			dom.removeClass(
				dom.select("td.mce-item-selected,th.mce-item-selected"),
				"mce-item-selected"
			);
		}
	...
	return function(editor) {
		...
		self.table = function(isProps) {
			...
			if (editor.settings.table_class_list) {
				...
				classListCtrl = {
					...
					type: "listbox",
					label: "Class",
					values: buildListItems(editor.settings.table_class_list, function(
						item
					) {
						if (item.value) {
		...
		self.cell = function() {
			...
			data = {
				...
				height: removePxSuffix(
					dom.getStyle(cellElm, "height") || dom.getAttrib(cellElm, "height")
				),
		...
		self.row = function() {
			...
			if (editor.settings.table_row_class_list) {
				classListCtrl = {
					...
					type: "listbox",
					label: "Class",
					values: buildListItems(editor.settings.table_row_class_list, function(
						item
					) {
						if (item.value) {
							item.textStyle = function() {
