		...
		function handleEnterKey(evt) {
			...
			function moveToCaretPosition(root) {
				...
				if (Env.ie && Env.ie < 9 && parentBlock && parentBlock.firstChild) {
					if (
						parentBlock.firstChild == parentBlock.lastChild &&
						parentBlock.firstChild.tagName == "BR"
					) {
				...
				if (root.hasChildNodes()) {
					...
					while ((node = walker.current())) {
						...
						if (
							moveCaretBeforeOnEnterElementsMap[node.nodeName.toLowerCase()]
						) {
			...
			function createNewBlock(name) {
				var node = container,
					...
					clonedNode,
					caretNode,
					textInlineElements = schema.getTextInlineElements();
			...
			function isCaretAtStartOrEndOfBlock(start) {
				...
				if (
					...
					(start ? offset > 0 : offset < container.nodeValue.length)
				) {
					return false;
				}
				...
				if (
					container.nodeName === "TABLE" ||
					(container.previousSibling &&
						container.previousSibling.nodeName == "TABLE")
				) {
					return (
			...
			function wrapSelfAndSiblingsInDefaultBlock(container, offset) {
				...
				if (!parentBlock || !canSplitBlock(parentBlock)) {
					...
					if (
						startNode &&
						schema.isValidChild(rootBlockName, blockName.toLowerCase())
					) {
						newBlock = dom.create(blockName);
						setForcedBlockAttrs(newBlock);
			...
			function insertNewBlockAfter() {
				...
				if (
					settings.end_container_on_empty_block &&
					canSplitBlock(containerBlock) &&
			...
			newBlockName =
				(settings.force_p_newlines ? "p" : "") || settings.forced_root_block;
			...
			if (parentBlockName == "PRE" && settings.br_in_pre !== false) {
				...
				if (
					(!newBlockName && !shiftKey && parentBlockName != "LI") ||
