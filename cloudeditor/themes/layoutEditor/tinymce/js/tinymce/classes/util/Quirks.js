	...
	return function(editor) {
		...
		function cleanupStylesWhenDeleting() {
			...
			function isTrailingBr(node) {
				...
				for (
					node = node;
					node != rootNode && !blockElements[node.nodeName];
			...
			function deleteRangeBetweenTextBlocks(rng) {
				...
				startBlock = dom.getParent(
					RangeUtils.getNode(rng.startContainer, rng.startOffset),
					dom.isBlock
				);
				endBlock = dom.getParent(
					RangeUtils.getNode(rng.endContainer, rng.endOffset),
					dom.isBlock
				);
				...
				if (
					!textBlockElements[startBlock.nodeName] ||
					!textBlockElements[endBlock.nodeName]
				) {
					return;
				}

				if (
					dom.getContentEditable(startBlock) === "false" ||
					dom.getContentEditable(endBlock) === "false"
				) {
					return;
				}
			...
			function handleLastBlockCharacterDelete(isForward, rng) {
				...
				function cloneTextBlockWithFormats(blockElm, node) {
					...
					currentFormatNodes = Tools.map(currentFormatNodes, function(
						formatNode
					) {
			...
			function customDelete(isForward) {
				...
				Tools.each(editor.getBody().getElementsByTagName("*"), function(elm) {
					...
					if (
						!elm.hasAttribute("data-mce-style") &&
						elm.hasAttribute("style")
					) {
						editor.dom.setAttrib(
							elm,
							"style",
							editor.dom.getAttrib(elm, "style")
						);
					}
				});
				...
				editor
					.getDoc()
					.execCommand(isForward ? "ForwardDelete" : "Delete", false, null);
				...
				Tools.each(mutationObserver.takeRecords(), function(record) {
					...
					Tools.each(record.addedNodes, function(node) {
						if (
							node.nodeName == "SPAN" &&
							!node.getAttribute("mce-data-marked")
						) {
			...
			editor.on("keypress", function(e) {
				if (
					...
					currentFormatNodes = $(rng.startContainer)
						.parents()
						.filter(function(idx, node) {
							return !!editor.schema.getTextInlineElements()[node.nodeName];
						});
		...
		function renderEmptyBlocksFix() {
			...
			if (getDocumentMode() >= 10) {
				...
				editor.contentStyles.push(
					emptyBlocksCSS + "{padding-right: 1px !important}"
				);
			}
		}
			...
			editor.on("drop", function(e) {
				if (!isDefaultPrevented(e)) {
					...
					if (internalContent && internalContent.id != editor.id) {
						...
						var rng = RangeUtils.getCaretRangeFromPoint(
							e.x,
							e.y,
							editor.getDoc()
						);
