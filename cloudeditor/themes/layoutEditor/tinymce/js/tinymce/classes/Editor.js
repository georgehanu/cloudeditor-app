...
define("tinymce/Editor", [
	...
	function Editor(id, settings, editorManager) {
		 ...
		 */
		self.documentBaseURI = new URI(
			settings.document_base_url || documentBaseUrl,
			{
				base_uri: baseUri
			}
		);
	...
	Editor.prototype = {
		...
		render: function() {
			...
			if (form) {
				...
				if (
					...
					!/TEXTAREA|INPUT/i.test(self.getElement().nodeName)
				) {
					DOM.insertAfter(
						DOM.create("input", { type: "hidden", name: id }),
						id
					);
			...
			function loadScripts() {
				...
				if (
					...
					!settings.language_url
				) {
					settings.language_url =
						self.editorManager.baseURL + "/langs/" + settings.language + ".js";
				}
				...
				if (
					settings.theme &&
					typeof settings.theme != "function" &&
					settings.theme.charAt(0) != "-" &&
					!ThemeManager.urls[settings.theme]
				) {
		...
		init: function() {
			...
			settings.aria_label =
				settings.aria_label ||
				DOM.getAttrib(elm, "aria-label", self.getLang("aria.rich_text_area"));
			...
			function initPlugin(plugin) {
				...
				pluginUrl =
					PluginManager.urls[plugin] || self.documentBaseUrl.replace(/\/$/, "");
				plugin = trim(plugin);
				...
				if (typeof settings.theme != "function") {
					...
					if (o.editorContainer.nodeType) {
						o.editorContainer = o.editorContainer.id =
							o.editorContainer.id || self.id + "_parent";
					}

					// Convert element type to id:s
					if (o.iframeContainer.nodeType) {
						o.iframeContainer = o.iframeContainer.id =
							o.iframeContainer.id || self.id + "_iframecontainer";
					}
			...
			if (settings.document_base_url != self.documentBaseUrl) {
				self.iframeHTML +=
					'<base href="' + self.documentBaseURI.getURI() + '" />';
			}
		...
		initContentBody: function(skipWrite) {
			...
			self.parser.addAttributeFilter("src,href,style,tabindex", function(
				nodes,
				name
			) {
				var i = nodes.length,
			...
			self.parser.addNodeFilter("p,h1,h2,h3,h4,h5,h6,div", function(nodes) {
				var i = nodes.length,
					node,
					nonEmptyElements = self.schema.getNonEmptyElements();
		...
		getLang: function(name, defaultVal) {
			return (
				this.editorManager.i18n.data[
					(this.settings.language || "en") + "." + name
		...
		getParam: function(name, defaultVal, type) {
			var value = name in this.settings ? this.settings[name] : defaultVal,
				output;
		...
		load: function(args) {
			...
			if (elm) {
				...
				html = self.setContent(
					elm.value !== undefined ? elm.value : elm.innerHTML,
					args
				);
				args.element = elm;
		...
		setContent: function(content, args) {
			...
			if (content.length === 0 || /^\s+$/.test(content)) {
				...
				if (
					...
					content = padd;
					content = self.dom.createHTML(
						forcedRootBlockName,
						self.settings.forced_root_block_attrs,
						content
					);
				} else if (!ie && !content) {
				...
				if (args.format !== "raw") {
					content = new Serializer(
						...
						self.schema
					).serialize(self.parser.parse(content, { isRootContent: true }));
				}
