...
tinymce.ThemeManager.add("modern", function(editor) {
	var self = this,
		...
		each = tinymce.each,
		DOM = tinymce.DOM,
		Rect = tinymce.ui.Rect,
		FloatPanel = tinymce.ui.FloatPanel;
	...
	function createMenuButtons() {
		...
		function createMenu(context) {
			...
			removedMenuItems = tinymce.makeMap(
	...
	function addContextualToolbars() {
		...
		function reposition(match) {
			...
			testPositions = ["tc-bc", "bc-tc", "tl-bl", "bl-tl", "tr-br", "br-tr"];
			...
			contentAreaRect = tinymce.DOM.getRect(
	...
	function renderIframeUI(args) {
		...
		panel = self.panel = Factory.create({
			...
			items: [
				settings.menubar === false
					? null
					: { type: "menubar", border: "0 0 1 0", items: createMenuButtons() },
				createToolbars(settings.toolbar_items_size),
				{
					type: "panel",
					name: "iframe",
		...
		if (settings.statusbar !== false) {
			panel.add({
				...
				border: "1 0 0 0",
				ariaRoot: true,
				items: [{ type: "elementpath" }, resizeHandleCtrl]
			});
		}
	...
	self.renderUI = function(args) {
		...
		editor.on("ProgressState", function(e) {
			self.throbber =
				self.throbber || new tinymce.ui.Throbber(self.panel.getEl("body"));
