...
tinymce.PluginManager.add("lists", function(editor) {
	...
	editor.on("init", function() {
		...
		function isEmpty(elm, keepBookmarks) {
			...
			if (
				keepBookmarks &&
				dom.select("span[data-mce-type=bookmark]").length > 0
			) {
				return false;
			}
		...
		function mergeWithAdjacentLists(listBlock) {
			...
			if (
				sibling &&
				isListNode(sibling) &&
				sibling.nodeName == listBlock.nodeName
			) {
		...
		editor.addQueryStateHandler(
			"InsertOrderedList",
			queryListCommandState("OL")
		);
		editor.addQueryStateHandler(
			"InsertDefinitionList",
			queryListCommandState("DL")
		);
